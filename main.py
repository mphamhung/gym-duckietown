...
parser.add_argument("--lrD", default=0.005, type=float, help="Discriminator learning rate")
...
parser.add_argument("--d-schedule", default=5, type=int, help="number of times to train D vs G, negative means train D every G updates")
...
parser.add_argument("--D-iter", default= 5, type=int)
...
parser.add_argument("--entropy-beta", default=0.01, type=float)
...
parser.add_argument("--train-vae",default = 0)
parser.add_argument("--use-vae", default= 0, type=int)
...
def main(args):
    ...
    G = Generator(observation_shape, action_dim, args.use_vae)
    G.to(device)
    D = Discriminator(observation_shape, action_dim, args.use_vae)
    D.to(device)
    ...
    if args.train:
        ...
        gail_agent.validation_set = gail_agent.get_expert_trajectories(1, 100, expert)
    ...
    if args.eval:
        ...
        traj = gail_agent.get_expert_trajectories(args.episodes, args.steps, expert)

        trained_actions = gail_agent.generator.get_means(traj["observations"].to(device))
        random_actions = gail_agent_random.generator.get_means(traj["observations"].to(device))
        trained_score = (traj["actions"].to(device) - trained_actions).norm(2)
        random_score = (traj["actions"].to(device) - random_actions).norm(2)
        print("For training sesh: {} \ntrained score {}, random score {}".format(args.training_name,trained_score,random_score))
